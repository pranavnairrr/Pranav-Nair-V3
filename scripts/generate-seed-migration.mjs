...
let sql = `-- Seeds the two pre-existing blog posts into the new posts table.
-- Applied via `;
sql += '`npx supabase db push`.\n\n';

for (const p of posts) {
  const json = htmlToTiptapJson(p.html);
  const jsonStr = sqlEscape(JSON.stringify(json));
  sql += `insert into posts (slug, title, excerpt, category, body_json, visibility, published_at)
values (
  '${sqlEscape(p.slug)}',
  '${sqlEscape(p.title)}',
  '${sqlEscape(p.excerpt)}',
...
writeFileSync('supabase/migrations/20260908140000_seed_posts.sql', sql);
console.log('written, length:', sql.length);
